...
var sanitizePostInput = function (req, res, next) {
...
var sanitizePatchInput = function (req, res, next) {
    if ('website' in req.body) req.body.website = validator.escape(req.body.website);
    if ('username' in req.body) req.body.username = validator.escape(req.body.username);
    if ('password' in req.body) req.body.password = validator.escape(req.body.password);
    if ('notes' in req.body) req.body.notes = validator.escape(req.body.notes);

    next();
};

var checkId = function (req, res, next) {
    if (!validator.isAlphanumeric(req.params.id)) return res.status(400).end("id must be alphanumeric");
    next();
};
...
router.delete('/:id/', utility.isAuthenticated, checkId, function (req, res) {
    db.passwords.findOne({
        ...
        if (err) return res.status(500).end(err);
        if (!password) return res.status(404).end("Password id: " + req.params.id + " does not exist.");
        if (password.author !== req.user._id) return res.status(403).end("forbidden");
...
router.patch('/:id/', utility.isAuthenticated, checkId, sanitizePatchInput, function (req, res) {
    db.passwords.findOne({
        ...
        var data = JSON.parse(crypto.decrypt(password.data, constants.masterKey));
        if ('website' in req.body) data.website = req.body.website;
        if ('username' in req.body) data.username = req.body.username;
        if ('password' in req.body) data.password = req.body.password;
